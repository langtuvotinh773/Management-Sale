...
namespace Management.Commons.SQL
{
    public class clsExecSQL
    {
        ...
        private readonly string CONNECTION_STRING = Program.config.ConnectionString;
        ...
        public int ExecuteNonQuery(string query, CommandType commandType, List<SqlParameter[]> parameters)
        {
            ...
            try
            {
                for (int i = 0; i < parameters.Count; i++)
                {
                    ...
                    //  row += Common.ParseInt(m_Command.Parameters["@Count"].Value);
                   this.m_Command.Parameters.Clear();
                }
        ...
        public string getDataParameters(List<SqlParameter[]> parameters)
        {
            string strData = string.Empty;
            //foreach (SqlParameter itemValue in parameters)
            //{
            //    strData = strData + itemValue.Value + ",";
            //}
        ...
        public int ExecuteNonQuery(string query, CommandType commandType, SqlParameter[] parameters, object outputParamName)
        {
            ...
            try
            {
                ...
                int row = Common.ParseInt(m_Command.Parameters[outputParamName.ToString()].Value);
